...
func (m *Matriz) Recorrer() {
	aux := m.Raiz
	for aux != nil {
		fmt.Println(aux.Dato.Carnet_Tutor, "->", aux.Dato.Carnet_Estudiante, "->", aux.Dato.Curso, "->")
		aux = aux.Siguiente
	}
}
...
func (m *Matriz) Reporte() {
	...
	if aux1 != nil {
		...
		for aux1 != nil {
			if flag_raiz {
				texto += "nodo" + strconv.Itoa(aux1.PosX+1) + strconv.Itoa(aux1.PosY+1) + "[label=\"" + aux1.Dato.Curso + "\" ,rankdir=LR,group=" + strconv.Itoa(aux1.PosX+1) + "]; \n"
				flag_raiz = false
			} else {
